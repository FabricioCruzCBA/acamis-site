	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header bg-primary text-white">
				<h5 class="modal-title" id="modalLabel">Cadastrar Parceiro</h5>
...
document.addEventListener('DOMContentLoaded', function () {
	...
	btnCadastrar.addEventListener('click', function () {
		...
		resetPreview();
		inserirCSRF();
		form.action = "{{ url('adm/parceiro/cad') }}";
		document.getElementById('modalLabel').textContent = 'Cadastrar Parceiro';
		modal.show();
	});
	...
	btnEditar.forEach(btn => {
		btn.addEventListener('click', function () {
			inserirCSRF();
			form.action = "{{ url('adm/parceiro/update') }}";
		...
		<div class="modal-content">
			...
			<form id="formExcluir" method="POST">
				<div class="modal-body">
					...
					<p class="mb-0">Tem certeza que deseja excluir este parceiro? Essa ação não pode ser desfeita!</p>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
...
document.addEventListener('DOMContentLoaded', function () {
	...
	document.querySelectorAll('.btnExcluir').forEach(btn => {
		btn.addEventListener('click', function () {
			...
			formExcluir.action = "{{ url('adm/parceiro/delete') }}";
			modalExcluir.show();
		});
	});
});
